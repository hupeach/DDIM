...
def betas_for_alpha_bar(t:int, alpha_bar, max_beta:float=0.02):
    """
...
def l2_loss(truth:torch.Tensor, pred:torch.Tensor):
    """
    计算l2损失
    :param truth:实际值
    :param pred: 预测值
    :return: l2损失
    """
    return torch.sum((pred-truth)**2,dim=(1,2,3)).mean()
...
def get_time_schedule(name:str,timesteps:int,n_steps:int,device):
    ...
    if name == 'linear':
        # 线性采集，逆序输出
        t = torch.linspace(0,timesteps-1,n_steps,dtype=torch.long)
