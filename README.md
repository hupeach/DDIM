...
本项目仓库在Jttor和Pytorch框架下实现了DDIM论文复现，并在Cifar10数据集（使用Jittor和Pytorch封装好的导入）上对二者进行实验，本项目仓库主要包括以下内容：
 ...
 - 模型训练：
	 ...
	 - 集成学习：训练了三个模型，做了一个集成，但是最后没有采用，因为比较影响采样速度，为了不影响FID测评，最后测试的模型是训练损失最小的模型，在测试集上计算10000张图片上计算FID。
 # 实验
...
- 实验结果发现与反思：
 ...
 - Jittor的使用观感
	 ...
	 - 类型：这是真踩了个大坑，float32和int64不兼容，报错会报一些奇奇怪怪的错误，然后让我提交issue，总之就不会定位到这里，打开同步之后也会奇奇怪怪的报错。
